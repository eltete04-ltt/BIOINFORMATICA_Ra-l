...
Descargo todos los paquetes: `dplyr`, `kableExtra` y `jsonlite` y los programas `datasets` y `dataformat`.
...
```{bash}
if [ ! -f ../../data/trypanosoma_annotated.txt ]; then
...
library('kableExtra')
genomas <- read.table('../../data/trypanosoma_annotated.txt',
                      header = TRUE, sep = '\t')
...
genomas[c(7,8), ]  
```
Cambia en el Assembly.Accession: la 7 es GCF_003719485.1 y la 8 es GCA_003719485.1
...
table(duplicados)
```
Esto significa que nignuna fila está duplicada.
...
```{bash}
if [ ! -e ../../data/ncbi_dataset/data/GCA_022059095.1/genomic.gff ]; then
   if [ ! -e ../../data/ncbi_dataset.zip ]; then
      ./datasets download genome accession GCA_022059095.1 --include genome,gff3 --filename ../../data/ncbi_dataset.zip
   fi
   unzip -o ../../data/ncbi_dataset.zip -d ../../data/
fi
```
...
#| eval: false
library('jsonlite')
prettify(readLines('../../data/ncbi_dataset/data/assembly_data_report.jsonl'))
```
...
gff <- read.table('../../data/ncbi_dataset/data/GCA_022059095.1/genomic.gff', sep = '\t',
...
:::{.callout-important icon=false title="Comprobaciones del dataframe"}
```{r}
dim(gff)
head(gff)
tail(gff)
...
region <- gff[gff$type == 'region', -3]
```
En lugar de generar objetos nuevos, se modifica cada objeto original. El código revisa las columnas y verifica si alguna fila contiene valores diferentes; si todas coinciden, esa fila se elimina.
:::

# **Hasta este punto es donde hemos llegado en clase. Debido a la longitud de esta práctica no voy a completarla entera.**

# **Los errores posteriores a este punto serán producto de compilar la práctica y no haber seguido con ella**
